# Day 07. BRIEF. Earnings Call Summariser.
...
![Local-only](https://img.shields.io/badge/Hosted-Local_only_(127.0.0.1:1007)-555555?style=flat-square)
![Status](https://img.shields.io/badge/Status-MVP_complete-16A34A?style=flat-square)

Local Flask web app + CLI for buy-side earnings call analysis. Drop in a transcript (.txt or .pdf) or pick a bundled sample; the tool detects speaker turns and roles, runs a deterministic hedge / certainty / deflection detector and a quantitative-claim regex, then makes one DeepSeek V4 call to extract guidance + themes + Q&A grid + risks + bull/bear verdict, all rendered in a Bloomberg-terminal aesthetic.

This is **Day 07 of a 30-day finance and AI portfolio sprint** where each
...
- **Cost target**: around $0.002 per call.
...
A browser window opens at `http://127.0.0.1:1007/`.
...
- `DEEPSEEK_MODEL_FAST` -- Optional override.
- `DAY07_EDGAR_UA` -- User-Agent for SEC EDGAR requests. SEC requires a real contact in the User-Agent header; replace the placeholder email with your own.
...
- Python 3.11+
- openai (DeepSeek client), openpyxl, matplotlib, PyMuPDF (fitz), python-pptx, reportlab, flask
...
day-07-earnings-summariser/
  server.py            Flask web server (port 1007)
...
Safwan Ahmed. Day 07 of the 30-day finance + AI sprint, 2026.
